...
- **RLS column grants** — the hard boundary. Only specific columns are writable at all (see Database above), name/category/brand/key/id/sort_order are never reachable from the browser, and there is no insert/delete policy on either table under any role.
- **DB-level CHECK constraints** (`supabase/schema.sql`) — `selling_price >= 0`, and generous length caps on every anon-writable text field (`description`, `title`, `eyebrow`, `about`, `image_url`, `hero_image_url`) and on `images` array length. These exist to stop a stray or malicious direct API write from leaving garbage (a negative price, megabytes of text) that no normal UI interaction would ever produce — not to restrict legitimate content, so keep the limits generous when touching them.
