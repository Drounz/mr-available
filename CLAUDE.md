...
Table `personas`, one row per "shop by what you need" collection (`key` = slug, primary key): `title`, `eyebrow`, `about`, `hero_image_url`, `sort_order`. This is owner-managed content — the app only ever reads and writes these columns, it never hardcodes persona copy anywhere in `index.html` or `admin.html`. Policies: public may SELECT every row; UPDATE on `title`/`eyebrow`/`about`/`hero_image_url` requires a real Supabase Auth sign-in (`to authenticated`, not `anon`) — this is the one write path in the whole app that isn't open by default, because it's reachable from `admin.html`'s "Page images" section (see below). No insert/delete from the browser either way; rows are seeded/added via the dashboard or `service-role` key.
...
A second static file, not linked from the storefront, for the owner to manage product photos, price, publish status, and (further down the page) each persona's hero image and copy. Two different security models live on this one page:
- **Product photos/description/price/published** — no login (deliberate choice, unchanged). Pick a product from the searchable list, then drop in one or more images — any filename works, since photos are stored per product folder: `products/<PRODUCT_ID>/<anything>.jpg` in the `products` Storage bucket. After each upload, the page lists that folder, rebuilds the public URLs, and saves the ordered array into that product's `images` column (`images[0]` is the card/primary photo). Delete and reorder (‹ ›) buttons on each thumbnail update both Storage and the database. A price field (`#priceInput` + Save) writes `selling_price`, and a switch (`#publishedToggle`) writes `published` immediately on change; the toggle refuses to turn a product on while its price is 0/blank, since the storefront hides zero-priced products anyway (`loadProducts()` filters `price>0` client-side on top of the `published=true` server-side filter). Keep the admin.html URL private regardless — signing in isn't the gate here, and price/publish status being editable from the page means there's real damage a stray link could do.
- **Page images** (`#pageImagesCard`) — requires a real sign-in via Supabase Auth (`client.auth.signInWithPassword`), because this section can write to the public `personas` table and `site` Storage bucket. The owner must have an auth user already (create one once via Supabase dashboard → Authentication → Users → Add user — this repo has no way to create that account for you). Signed out, only an email/password form shows; signed in, it lists every row from `personas` (not hardcoded to five, so adding a row later — even something unrelated to a persona, like a future homepage hero — just appears here) with a hero-image preview, an Upload/Replace control, and editable eyebrow/title/about fields with Save. Uploads go to the `site` Storage bucket at `personas/<key>.<ext>` (stable path — re-uploading replaces it) and the resulting public URL is written to that row's `hero_image_url`.
