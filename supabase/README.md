...
3. `admin.html` is part of the deployed site (e.g. `yoursite.com/admin.html`) but not linked from the storefront and has no login — keep the link to yourself. Search for a product and drop in one or more photos — any filename works, since photos live in a folder per product (`products/<PRODUCT_ID>/...` in Storage) rather than needing to match the ID themselves. The first photo is the card image; add more for a gallery. Delete and reorder buttons appear on each thumbnail. A description box on the same screen sets the short spec line shown on the product's page.
...
- **Add photos or a description**: use `admin.html` (see section 4). It handles Storage uploads, the `images`/`image_url` columns, and `description` for you — editing those by hand in the Table Editor works too, but you'd need to build the public URLs yourself for photos.
