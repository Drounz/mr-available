...
2. Paste the contents of `supabase/schema.sql` and click **Run**.
   ...
   - `products`: anyone can read every product and update its photos, name, description, selling price, or published status, but never its category, brand, model, `personas`, or id, and can create a new product (id/name/category/brand/model only — it starts at ₦0 and unpublished) but never delete one, from the browser. No login is required by deliberate choice — `admin.html` has no sign-in, so keep its URL private; that matters more now that price, publish status, and product creation are all editable from the page.
...
3. `admin.html` is part of the deployed site (e.g. `yoursite.com/admin.html`) but not linked from the storefront and has no login anywhere on the page — keep the link to yourself. An **Add a product** box near the top creates a new product (ID, name, category, brand, optional model) — it starts unpublished at ₦0 with no photos, and gets auto-selected below so you can finish setting it up right away. To edit an existing one, search for it and drop in one or more photos — any filename works, since photos live in a folder per product (`products/<PRODUCT_ID>/...` in Storage) rather than needing to match the ID themselves. The first photo is the card image; add more for a gallery. Delete and reorder buttons appear on each thumbnail. A name field lets you fix a typo or rename it. A description box sets the short spec line shown on the product's page. A price field sets `selling_price`, and a Published switch flips the product live or hidden — the switch won't turn on while the price is still ₦0, since a zero-priced product never shows on the storefront anyway.
...
- **Add a product**: use `admin.html`'s "Add a product" box (see section 4) — ID, name, category, brand, optional model. Or Table Editor > products > insert row directly, using the product code as `id`; either way it starts unpublished at ₦0 until priced and published.
- **Rename a product, or fix category/brand/model**: the name field in `admin.html` handles renames; category/brand/model can only be set at creation from the browser, so fix a typo in those by hand in the Table Editor.
